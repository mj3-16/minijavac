class BalancedParens {
  ...
  public boolean isBalanced() {
    int i = 0;
    while (i >= 0 && i < size) i = recursiveCheck(i);
    return i == size;
  }

  /**
   * @return -1 if we run out of parens to check or the position where the paren at pos is closed +
   *     1
   */
  public int recursiveCheck(int pos) {
    if (pos + 1 >= size) return -1;
    if (parens[pos] != 1) return -1;
    pos = pos + 1;
    while (pos >= 0 && parens[pos] != 0) pos = recursiveCheck(pos);
    return pos + 1;
  }
  ...
  public static void main(String[] args) {
    ...
    test.parens[19] = 0; /* (()(()))()()(((()))()) */
    /* Print 1 */
    if (test.check()) System.out.println(1);
